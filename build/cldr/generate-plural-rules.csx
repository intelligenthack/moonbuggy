...
// Stores CLDR conditions as C# expression strings for use by the source generator.
...
foreach (var kvp in allLocales.OrderBy(k => k.Key))
{
    ...
    foreach (var cat in CatOrder)
    {
        ...
        var cond = EmitCondition(rules[cat]);
        if (cond != null)
            entries.Add($"{{ PluralCategory.{cat}, \"{cond.Replace("\"", "\\\"")}\" }}");
    }
    if (entries.Count > 0)
    {
